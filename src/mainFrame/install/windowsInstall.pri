...
QMAKE_POST_LINK += && \
    ...
    xcopy /y /s /e /i  \"$${TRD}\protobuf\lib\\$${PLATFORM}\\$${BUILD_MODE}\*.dll\"     \"$${DESTDIR}\"                 && \
    xcopy /y /s /e /i  \"$${TRD}\glog\lib\\$${PLATFORM}\\$${BUILD_MODE}\*.dll\"         \"$${DESTDIR}\"                 && \
    xcopy /y /s /e /i  \"$${TRD}\7z1900-extra\\$${PLATFORM}\*.*\"                       \"$${DESTDIR}\"                 && \
    xcopy /y /s /e /i  \"$${TRD}\openssl\\$${PLATFORM}\*.dll\"                          \"$${DESTDIR}\"

# resource
QMAKE_POST_LINK += && \
    xcopy /y /i  \"$${SRC}\mainFrame\*.qm\"                 \"$${MAINFRAME}\language\"   && \
    xcopy /y /i  \"$${SRC}\mainFrame\mainFrame\*.ico\"      \"$${MAINFRAME}\*.ico\"
